  ...
  return (
    <div className="mx-auto xl:w-7/12 lg:w-10/12 md:w-12/12"> 
      ...
      <div className=" grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-7 px-5 bg-slate-800">
            ...
            {pokemonDetails.filter((pokemon) => {pokemon.name.toLowerCase().includes(searchText.toLowerCase())}).map((pokemon) => {asd})}
      {pokemonDetails.filter((pokemon) => {pokemon.name.toLowerCase().includes(searchText.toLowerCase())}).map((pokemon, index) => (
